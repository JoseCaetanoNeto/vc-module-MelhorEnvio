...
namespace vc_module_MelhorEnvio.Core
{
    ...
    public abstract class OAuth2
    {
        ...
        protected OAuth2(Dictionary<string, string> configuration, HttpContext pHttpContext, string pUserAgent = null)
        {
            m_UserAgent = pUserAgent;
            m_HttpContext = pHttpContext;
        ...
        public string makeRequest(string path, Dictionary<string, string> parameters, string method = "GET")
        {
            ...
            request.UserAgent = string.IsNullOrEmpty(m_UserAgent) ? m_UserAgent : "oauth2-draft-v10";
        ...
        public string getAccessTokenFromAuthorizationCode(string code)
        {
            var p = new Dictionary<string, string>() {
                ...
                { "client_secret", _variables["client_secret"] },
                {"code", code},
        ...
        public string getSession()
        {
            ...
            if (session == null && _variables.ContainsKey("code"))
            {
                ...
                var a = JsonConvert.DeserializeObject<Access>(json);
        ...
        {
            m_HttpContext.Response.Cookies.Append("oauth2_" + getVariable("client_id"), getQueryString(session));
        }
        ...
        {
            var cookie = m_HttpContext.Request.Cookies["oauth2_" + getVariable("client_id")];
            if (cookie == null)
            {
                throw new NotImplementedException("need to get back to Auth page");
            }
            var l = HttpUtility.ParseQueryString(cookie);
        ...
        protected Dictionary<string, string> getSessionObject(Access a)
        {
            var session = new Dictionary<string, string>();
            if (a != null)
            {
                ...
                session.Add("refresh_token", a.refresh_token);
                session.Add("token_type", a.token_type);
